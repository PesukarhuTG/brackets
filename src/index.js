module.exports = function check(str, bracketsConfig) {
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    if (closeBrackets.indexOf(str[i]) > -1) {
      ...
      //и если в паре ключ-значение, значение закрывающего символа соотвествует значению открывающего симовала,
      //то из стека удаляем этот символ. Если нет, то проверка не выполнена
      if (brackets[str[i]] === topElement) {
        stack.pop();
      } else {
        return false;
      }
